...
export function FnacHeader({ title, subtitle, icon: Icon, children }: FnacHeaderProps) {
  ...
  return (
    <header className="border-b" style={{ background: "hsl(var(--sidebar-bg))" }}>
      <div className="w-full px-2 sm:px-4 py-2 max-sm:landscape:py-1 sm:py-3 flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 flex-wrap">
          <button onClick={() => { handleLogoClick(); navigate((role === "admin" || role === "editor" || role === "manager") ? "/" : "/equipe-du-jour"); }} className="flex items-center gap-1.5 hover:opacity-80 transition-opacity relative shrink-0">
