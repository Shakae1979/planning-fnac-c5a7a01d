...
export function ScheduleEditor() {
  ...
  // Rôle du jour (collaborateurs multi-métiers) — avec plages horaires
  ...
  const saveDayRanges = async (empId: string, dayIndex: number, ranges: { role: string; start: string; end: string }[]) => {
    ...
    try {
      ...
      if (ranges.length > 0) {
        const { error } = await (supabase as any).from("employee_day_roles").insert(
          ranges.map((r) => ({ employee_id: empId, date, role: r.role, start_time: r.start, end_time: r.end }))
        );
        if (error) throw error;
      }
  ...
  const resetWeekMutation = useMutation({
    mutationFn: async () => {
      ...
      const ids = schedules
        .filter((s) => visibleIds.has(s.employee_id))
        .map((s) => s.id);
                        ...
                        return (
                          <td key={`${day.key}-cell`} colSpan={2} className={`py-0.5 px-0.5 ${isRoulement ? "bg-muted/60" : isCellSource ? "bg-primary/10" : ferieDay ? "bg-muted/50" : ""}`}>
                            ...
                            {(() => {
                              ...
                              return (
                                <DayRoleEditor
                                  rows={dayRoleMap[`${emp.id}__${dayDate}`]}
                                  mainRole={emp.role}
                                  secondaries={secondaries}
                                  shiftStart={startVal}
                                  shiftEnd={endVal}
                                  onSave={(ranges) => saveDayRanges(emp.id, dayIndex, ranges)}
                                />
                              );
                            })()}
